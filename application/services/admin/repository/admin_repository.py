...
class AdminRepository(BaseRepository):
    async def create_readonly_user(
        ...
        try:
            ...
            await self.execute(
                text(f"CREATE USER \"{safe_username}\" WITH PASSWORD '{safe_password}' VALID UNTIL '{expiration_date_str}'")
            )
            
            # Назначаем права только на чтение для всех таблиц в схеме public
            await self.execute(
                text(f"GRANT USAGE ON SCHEMA public TO \"{safe_username}\"")
            )
            
            await self.execute(
                text(f"GRANT SELECT ON ALL TABLES IN SCHEMA public TO \"{safe_username}\"")
            )
            ...
            LOGGER.info(
                f"Successfully created read-only user '{username}' with expiration date: {expiration_date_str}"
            )
    ...
    async def revoke_user_access(self, username: str) -> None:
        ...
        try:
            ...
            # Отзываем все права
            await self.execute(
                text(f"REVOKE ALL PRIVILEGES ON SCHEMA public FROM \"{safe_username}\"")
            )
            
            await self.execute(
                text(f"REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM \"{safe_username}\"")
            )
            
            # Устанавливаем дату истечения на текущую дату, чтобы немедленно отозвать доступ
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            await self.execute(
                text(f"ALTER USER \"{safe_username}\" VALID UNTIL '{current_time}'")
            )
    ...
    async def check_user_exists(self, username: str) -> bool:
        ...
        try:
            ...
            result: Result = await self.execute(
                text(f"SELECT 1 FROM pg_roles WHERE rolname = '{safe_username}'")
            )
    ...
    async def extend_user_access(self, username: str, new_expiration_date: datetime) -> None:
        ...
        try:
            ...
            await self.execute(
                text(f"ALTER USER \"{safe_username}\" VALID UNTIL '{expiration_date_str}'")
            )
