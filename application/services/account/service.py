...
class AccountService:
    ...
    async def get_info(self, token: str) -> UserDetailsEnvelope:
        login = await self.account_cache.get_login(token)
        if not login:
            async with service_error_handler():
                r = await self.account_api.get_v1_account_with_http_info(x_dm_auth_token=token)
                response = UserDetailsEnvelope.model_validate_json(r.content)
            if hasattr(response, "resource") and response.resource is not None:
    ...
    async def reset_password(self, reset_password_model: ResetPassword) -> UserEnvelope:
        async with service_error_handler():
            response = await self.account_api.post_v1_account_password_with_http_info(
                reset_password=reset_password_model
            )
            return UserEnvelope.model_validate_json(response.content)

    async def change_password(self, change_password_model: ChangePassword) -> UserEnvelope:
        change_password_model.token = str(change_password_model.token)
        async with service_error_handler():
            response = await self.account_api.put_v1_account_password_with_http_info(
                change_password=change_password_model
            )
            return UserEnvelope.model_validate_json(response.content)

    async def change_email(self, change_mail_model: ChangeEmail) -> UserEnvelope:
        async with service_error_handler():
            response = await self.account_api.put_v1_account_email_with_http_info(change_email=change_mail_model)
            return UserEnvelope.model_validate_json(response.content)

    async def delete_account(self, token: str, email: str) -> None:
        response = await self._user_info_by_token(token=token)
        if hasattr(response, "resource") and response.resource is not None:
            login = response.resource.login
            if not login:
            ...
            dataset = await self.account_repository.get_user(login=login)
            ...
            await self.account_cache.set_delete_account_token(
                token=token, delete_token=str(delete_token).encode("utf-8")
            )
            return
        raise AuthorizationError

    async def delete_account_by_token(self, token: str, delete_token: str) -> str:
        response = await self._user_info_by_token(token=token)
        if hasattr(response, "resource") and response.resource is not None:
            login = response.resource.login
            if not login:
            ...
            if confirmation_token == delete_token:
                await self.account_repository.delete_account(login=login)
                return "ok"
            return "error"
